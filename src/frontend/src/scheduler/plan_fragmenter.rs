...
impl BatchPlanFragmenter {
    ...
    pub fn split(
        ...
        let root_stage = self.new_stage(
            batch_node.clone(),
            Distribution::Single.to_prost(1, &self, snapshot_align_epoch),
    ...
    fn new_stage(
        ...
        let parallelism = match root.distribution() {
            ...
            _ => match &table_scan_info {
                ...
                {
                    if let Some(lookup_join_parallelism) = self
                        .collect_stage_lookup_join_parallelism(root.clone(), snapshot_align_epoch)?
                    {
    ...
    fn visit_exchange(
        ...
        let mut execution_plan_node = ExecutionPlanNode::from(node.clone());
        let child_exchange_info =
            node.distribution()
                .to_prost(builder.parallelism, self, snapshot_align_epoch);
    ...
    fn collect_stage_lookup_join_parallelism(
        ...
        if let Some(lookup_join) = node.as_batch_lookup_join() {
            ...
            node.inputs()
                .into_iter()
                .find_map(|n| {
                    self.collect_stage_lookup_join_parallelism(n, snapshot_align_epoch)
                        .transpose()
                })
                .transpose()
        }
